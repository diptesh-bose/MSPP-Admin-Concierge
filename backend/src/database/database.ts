...
async function insertDefaultData(): Promise<void> {
  ...
  for (const category of categories) {
    await db.run(
      'INSERT INTO compliance_categories (name, description, icon, order_index) VALUES (?, ?, ?, ?)',
      [category.name, category.description, category.icon, category.order_index]
    );
  }
  ...
  // Insert default CLI commands
  await insertDefaultCLICommands();

  logger.info('Default data inserted successfully');
}

async function insertDefaultEnvironments(): Promise<void> {
  if (!db) throw new Error('Database not initialized');
  ...
  for (const env of defaultEnvironments) {
    await db.run(
      'INSERT INTO environments (id, name, display_name, type, region, created_by) VALUES (?, ?, ?, ?, ?, ?)',
      [env.id, env.name, env.display_name, env.type, env.region, env.created_by]
    );
  }

  logger.info('Default environments inserted successfully');
}

async function insertDefaultComplianceItems(): Promise<void> {
  if (!db) throw new Error('Database not initialized');
...
async function insertDefaultCLICommands(): Promise<void> {
  if (!db) throw new Error('Database not initialized');
...
async function runMigrations(): Promise<void> {
  ...
  try {
    // Check if completion_details column exists
    ...
    if (!hasCompletionDetails) {
      ...
      logger.info('Added completion_details column to compliance_items table');
    }

    // Check if environment_id column exists in compliance_items
    ...
    if (!hasEnvironmentId) {
      ...
      logger.info('Added environment_id column to compliance_items table');
    }

    // Check if environment_id column exists in audit_logs
    ...
    if (!auditHasEnvironmentId) {
      ...
      logger.info('Added environment_id column to audit_logs table');
    }
    ...
    if (tables.length === 0) {
      ...
      logger.info('Created environments table and indexes');
    }
