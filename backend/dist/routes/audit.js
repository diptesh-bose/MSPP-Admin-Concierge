...
router.get('/summary', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const db = (0, database_1.getDatabase)();
    const [totalActions, actionsByType, activityTrend, topUsers, recentActivity] = await Promise.all([
        db.get(`
...
router.post('/logs', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { action, resource_type, resource_id, user_id, details, ip_address, user_agent } = req.body;
...
router.get('/stats', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const db = (0, database_1.getDatabase)();
    const [todayStats, weekStats, monthStats, resourceTypeStats] = await Promise.all([
        db.get(`
      SELECT COUNT(*) as count
      FROM audit_logs
...
router.get('/export', (0, errorHandler_1.asyncHandler)(async (req, res) => {
  ...
  `;
    ...
    if (format === 'csv') {
        ...
        const csvRows = formattedLogs.map((log) => `${log.id},"${log.action}","${log.resource_type}","${log.resource_id || ''}","${log.user_id || ''}","${log.created_at}","${JSON.stringify(log.details || {})}"`).join('\n');
